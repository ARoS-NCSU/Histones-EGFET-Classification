...
def separate_by_cycle(
    ...
    # find where there are negative discontinuities in gate voltage
...
def load_egfet_dataset(
    ...
    Each folder has the data for a different analyte and aptamer. The folder name
    will be used to identify the trial, and each data file in the folder will be loaded.
    Every folder starts with a date, followed by the analyte, then the aptamer, then the concentration

    Args:
        data_path (str): _description_
        exclude_zero_concentration (bool): _description_
        flatten (bool): _description_
    Returns:
        Dict[str, pd.DataFrame]: Dictionary of multi-indexed dataframe with the data organized by concentration
    """
    # find valid folders
    verbose_ = verbose > 0
    folders = [
